...
void simulation() {
	//clock_t start=clock();
	int number_of_events=1000000;
	int point_of_noise=10;
	...
	cout<<" Multiple Scattering is "<<multScattering<<" (0 = Off, 1 = On);"<<endl;
	cout<<" Noise function is "<<noise<<"(0 = Off, 1 = On) and there are "<<point_of_noise<<" hits due to the noise for layer and for event"<<endl;
	cout<<" Do you want to modify the default parameters? (0 = No, 1 = Yes) ";
	int def = 0;//default condition flag
	cin>>def;

	if(def == 1){
		cout<<endl<<"Please insert the number of events: ";
		cin>>number_of_events;
		cout<<"  Please insert '1' to switch on multiple scattering, '0' to switch off: ";
		cin>>multScattering;
		cout<<"  Please insert '1' to switch on noise function, '0' to switch off:"; 
		cin>>noise;
		if(noise==1){
			cout<<"  Please insert the number of hits due to the noise:";
			cin>>point_of_noise; 
			}
	}

	//cout<<endl<<" Now creating "<<events<<" events"<<endl;
...
*/
	...
	for(int i = 0; i < number_of_events; i++) { // loop sugli eventi
		...
		messi dentro al ciclo degli eventi.*/
			...
			if(noise) {
				add_noise(point_of_noise, hits1, det1, count_hit1, hit_det1); 
				add_noise(point_of_noise, hits2, det2, count_hit2, hit_det2);
			}
