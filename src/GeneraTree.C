...
public:
	...
	~ClonesArray() {
		delete ptr; //perchè serve fare delete
	}
...
void GeneraTree() {
	...
	//Dati di partenza: lunghezza rivelatori e caratteristiche
	double lunghezza = 27;
	hit hits1(0, 0, 0), 
		hits2(0, 0, 0),
		hitBP(0, 0, 0);
	...
	for(int i = 0; i < NUMBER_OF_EVENTS; i++) { // loop sugli eventi
		...
		for(int j = 0; j < point.mult; j++) {
			...
			if(multScattering){
				...
				//Si verifica che il punto di intersezione su BP soddisfi accettanza,si cambia direzione e si sostituiscono i valori di vtx con quelli dell'intersezione:
				//se non è soddisfatta la condizione, non si riempe nulla: non ci sono HIT nè sul primo layer, nè sul secondo.

				if(hitBP.accettanza(lunghezza)){

					 direction.scattering();//cambio angoli in seguito a MS

					vtx_hit.X = hitBP.getX();//Cambio vertice
					vtx_hit.Y = hitBP.getY();
					vtx_hit.Z = hitBP.getZ();

					Particella& changedirect_bp = *(new(scatter_bp.array[count_bp]) Particella(direction));

					//cout << "\nDirezione # " << changedirect_bp.getLabel() << ": phi = " << changedirect_bp.getPhi() << ": theta = " << changedirect_bp.getTheta() << endl;

					count_bp++;

					hits1.intersezione(vtx_hit, det1, changedirect_bp);
				//Una volta determinato il punto di intersezione sul primo layer, si controlla che l'accettanza sia soddisfatta e solo in quel caso si riempe hit_det1
				//se non sono dentro al primo non posso essere dentro al secondo, infatti man mano che mi allontano dal vertice sono sempre più esterno come punto su asse z
					if(hits1.accettanza(lunghezza)){
						cout <<"Z1:"<<endl;
						hits1.PrintStatus();

						changedirect_bp.scattering();
				...
				//SMEARING:
				// i valori deltaz e deltaphi vanno a modificare i punti di intersezione con una gaussiana avente media il punto trovato e sigma la delta fornita,
				//in questo modo si in considerazione la risposta reale del rivelatore.
						...
						hits2.intersezione(vtx_hit, det2, changedirect_1);
				//Una volta determinato il punto di intersezione sul secondo layer, si controlla che l'accettanza sia soddisfatta e solo in quel caso si riempe hit_det2
		...
		//Se MS è FALSE si passa direttamente qui dove si effettuare prima l'intersezione sul primo layer, si verifica accettanza,si valuta smearing e si riempe hit_det1.
		//Se si è passato il primo rivelatore si passerà poi al secondo e si controlla accettanza, smearing e si riempe hit_det2
		...
		//RUMORE: punti aggiuntivi che si mettono per ogni evento simulato oltre ai conteggi veri. Sono conteggi spuri dovuti all'elettronica.
		//Se non dovessi soddisfare le condizioni di accettanza del primo detector(senza MS) o della BP(con MS) si salverebbero solo i conteggi di noise che sono messi dentro al ciclo degli eventi.
