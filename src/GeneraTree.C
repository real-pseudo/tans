...
Particella *p=new Particella();
  ...
  TClonesArray *ptrdirez = new TClonesArray("Direzione",100); //vettore di classi
  TClonesArray& direz = *ptrdirez; //direz è un puntatore ad oggetti della classe Tclones array è una copia di ptrdirez
 TClonesArray *ptrHIT1 = new TClonesArray("hit",100);
  TClonesArray& HIT1 = *ptrHIT1;
  TClonesArray *ptrHIT2 = new TClonesArray("hit",100);
  ...
  typedef struct {
	...
	Cilindro *riv1=new Cilindro(1,4.,0.02);
	Cilindro *riv2=new Cilindro(2,7.,0.02);
  ...
  for(Int_t i=0; i<10;i++){ // loop sugli eventi
    ...
    while(numpart<=0){
      ...
      cout<<i<<"__ "<<numpart<<endl;
		...
		cout<<"Evento: "<<i<<"----"<<"molteplicità"<<point.mult<<endl;
		int a=0;
		int b=0;
//loop su ogni evento
		for(int j=0;j<point.mult;j++){
			new(direz[j])Direzione(j,p->phi(),p->theta());
			Direzione *tst=(Direzione*)ptrdirez->At(j);
			cout<<"Direzione"<<j<<") j, phi, theta = "<<tst->GetLabel()<<"; "<<tst->GetPhi()<<"; "<<tst->GetTheta()<<endl;
		//per ogni particella devo controllare la condizione su z
			...
			if((hits1.condizione(lunghezza))==0){
				//cout<<"ok"<<endl;
				new(HIT1[a])hit(hits1.GetX(),hits1.GetY(),hits1.GetZ());
				a++;
				
			}
			if((hits2.condizione(lunghezza))==0){
				//cout<<"ok!!!!!!!!"<<endl;
