...
//varibili utili alla ricostruzione
  ...
  double deltaphi;
  TH1D *z = new TH1D("z","istogramma ",400,-15,15);
  z->SetDirectory(0);
 ...
 double width = 0.001;
    ...
    for(int v=0;v<nbin+1;v++){
      if(trackZ->GetBinContent(v) == trackZ->GetBinContent(bin_peak) && v!=bin_peak ) {
    	 /*cout<<"pippo1"<<endl;
    	 cout<<trackZ->GetBinContent(v)<<"_______"<<v<<endl;
    	 cout<<trackZ->GetBinContent(bin_peak)<<endl;*/
    	 twopeaks=true;
       //c++;
      }
    }
...
*/
    	  ...
    	  //c++;
        cout << "vtx originale:"<< point.Z << " -- vtx NONNNNNN ricostruito????: "<< most_prob_Z <<endl;
      }
    delete trackZ;
